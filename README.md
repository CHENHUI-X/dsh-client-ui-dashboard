...
| 请求趋势 / Request trend | Row 1: **gradient area chart** of per-request cache hit rate (hover shows a crosshair + exact %; running/zero-input samples are skipped so no fake 0% drops) + interactive input/output/duration **bar charts** (hover grows the bar and floats the exact value; running bars pulse; each chart keeps a fixed semantic color). Row 2: **TTFT area chart** + **input-per-turn area chart** (delta badge + compaction markers) + **reasoning-share area chart** + **cache-write bar chart**. Row 3: **request & token density** (minute buckets). A "last 60" note appears when there are more requests (totals explain their scope on hover). The request table — **view tabs (table / by turn / by model / by error with collapsible groups + aggregates), status filter, live search (seq / turn / model / error / purpose / tool name), JSON export, paging** — rounds out the section |
